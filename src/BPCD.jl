...
function BPCD(particle_count::T, hash_table_size::T, max_radius::Number,
              domain_min::AbstractVector, type::BPCDType.T) where {T <: Integer}
...
function BPCD(particle_count, max_radius, domain_min::AbstractVector,
              domain_max::AbstractVector, type::BPCDType.T = BPCDType.Implicit)
...
function cell(pos, domain_min, cell_size)
    cartesian3morton(@. floor(Int32, (pos - domain_min) / cell_size) + Int32(1))
end

function count_particles!(domain_min, cell_size, cell_count, positions)
    index = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    stride = gridDim().x * blockDim().x
    for i in index:stride:length(positions)
...
function setup!(bpcd::BPCD, positions)
    ...
    CUDA.@sync @cuda threads=256 blocks=ceil(Int, length(positions) / 256) count_particles!(bpcd.domain_min,
                                                                                            bpcd.cell_size,
                                                                                            bpcd.cell_count,
                                                                                            positions)
